...
function scl_menu_checklist_status_label( $title, $page_id ) {
	...
	$current_screen = function_exists( 'get_current_screen' ) && get_current_screen() !== null ? get_current_screen()->base : null;

	if ( is_admin() && 'nav-menus' == $current_screen ) {
...
function scl_menu_screen_add_private_pages( $query ) {
	...
	if ( is_admin() && 'nav-menus' == $current_screen_ob ) {
